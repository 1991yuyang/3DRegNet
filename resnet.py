...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=(1, 3), stride=stride,
                     padding=(0, dilation), groups=groups, bias=False, dilation=dilation)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(1, self.inplanes, kernel_size=(1, 7), stride=1, padding=(0, 3),
                               bias=False)
        # self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=(1, 3), stride=1, padding=(0, 1))
        self.layer1 = self._make_layer(block, 1, layers[0])
        self.layer2 = self._make_layer(block, 1, layers[1], stride=1,
